...
$project = "andre98652_teammates-frontend-clean"        # CAMBIO: nombre del proyecto frontend
...
Write-Host "Descargando métricas generales (FRONTEND)..."
...
$respM | ConvertTo-Json -Depth 5 | Out-File -Encoding UTF8 "measures_frontend.json"
Write-Host "Guardado: measures_frontend.json"
...
# DESCARGAR ISSUES
Write-Host "`nDescargando issues (FRONTEND)..."
...
$issues | Select key,type,severity,rule,message,component,line,creationDate,updateDate,status,resolution,debt |
    Export-Csv -Path "issues_frontend_all.csv" -NoTypeInformation -Encoding UTF8

$hotspots | Select key,type,severity,rule,message,component,line,creationDate,updateDate,status,resolution |
    Export-Csv -Path "hotspots_frontend_all.csv" -NoTypeInformation -Encoding UTF8

Write-Host "`nExportaciones completadas:"
Write-Host "→ issues_frontend_all.csv"
Write-Host "→ hotspots_frontend_all.csv"
Write-Host "→ measures_frontend.json"
...
Write-Host "Hotspots exportados a hotspots_frontend_all.csv"
