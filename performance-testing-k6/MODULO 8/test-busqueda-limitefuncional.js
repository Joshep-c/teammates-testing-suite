import http from 'k6/http';
import { check } from 'k6';

export const options = {
  stages: [
    { target: 20, duration: '20s' },    // Carga inicial moderada
    { target: 50, duration: '20s' },    // Escalado progresivo
    { target: 100, duration: '30s' },   // Carga media
    { target: 200, duration: '30s' },   // Carga alta
    { target: 300, duration: '30s' },   // Límite funcional estimado
    { target: 0, duration: '20s' },     // Descenso
  ],
};

export default function () {
  const url = 'https://modern-vortex-463217-h9.uc.r.appspot.com/webapi/search/students?searchkey=Alice&entitytype=instructor';

  const headers = {
    'Accept': 'application/json, text/plain, */*',
    'X-WEB-VERSION': '8.8.0',
    'ngsw-bypass': 'true',
    'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64)',
    'Referer': 'https://modern-vortex-463217-h9.uc.r.appspot.com/web/instructor/search',
    'Cookie': 'AUTH-TOKEN=; CSRF-TOKEN=E5797A8B2F31C4C8CD2B1479E8A32819B603073E19F6DBA99DA6CCAE39A39533; JSESSIONID=node0948tyv8homb7cgbeejkz8ix610.node0'
  };

  const res = http.get(url, { headers });

  check(res, {
    'status is 200': (r) => r.status === 200,
    'body contiene resultados': (r) => r.body && r.body.includes('Alice'),
  });
}
