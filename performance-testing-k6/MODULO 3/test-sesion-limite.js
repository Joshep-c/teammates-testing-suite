...
export default function () {
  ...
  // Fecha de visibilidad y envío
  const now = new Date();
  const nextHour = new Date(now);
  nextHour.setHours(now.getHours() + 1, 0, 0, 0);

  // Nombre de sesión al límite (255 caracteres)
  const feedbackSessionName = 'X'.repeat(255);

  // Instrucciones extensas (5000 caracteres)
  const instruccionesLargas = 'Y'.repeat(5000);

  // Payload con campos en el borde de los límites
  const payload = JSON.stringify({
    feedbackSessionName,
    timeZone: 'America/Lima',
    instructions: `<p>${instruccionesLargas}</p>`,
