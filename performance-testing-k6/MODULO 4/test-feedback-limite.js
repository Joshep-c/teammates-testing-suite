...
export default function () {
  ...
  const longAnswer = '<p>' + 'A'.repeat(5000) + '</p>';
  const longRecipient = 'estudiante-extremo-' + 'Z'.repeat(220) + '@example.com';
  ...
  check(res, {
    'Status 200 con campos límite': (r) => r.status === 200,
    'Body válido con respuesta larga': (r) => r.body && r.body.length > 0,
  });

  console.log('Boundary test status:', res.status);
}